...
// Load .env from THIS file's folder (backend/), not from wherever the
// script happens to be run from — fixes seed.js silently connecting to
// the wrong (local) database when run as `npm run seed` from the root.
...
async function seed() {
  ...
  console.log(`🔗 Connecting to: ${isLocal ? 'LOCAL MongoDB (fallback — .env not loaded!)' : 'Atlas / remote (from .env)'}`);
  ...
  const adminUser = await new User({
    name:     'Udeesha Jayendra',           // ← Change this
  ...
  await Profile.create({
    name:     'Udeesha Jayendra',            // ← Change
    title:    'Networking Enthusiast ', // ← Change
    tagline: 'Undergraduate | Network Enthusiast | Aspiring Network Engineer',
    bio: 'Undergraduate with a passion for computer networking, network infrastructure, and troubleshooting. Eager to gain practical industry experience and develop professional networking skills through an internship.',
    email:    'testudeesha@gmail.com', // ← Change
